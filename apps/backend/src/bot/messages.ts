...
export const STEP_MESSAGES: Record<SurveyStep, string> = {
  ...
  references: '👥 *25/28* Tavsiya bera oladigan 2 nafar shaxsning F.I.Sh. va telefon raqami (ixtiyoriy).\n\nJavob yozing yoki "O\'tkazib yuborish" tugmasini bosing:',
...
export const SKIP_STEP_LABEL = "O'tkazib yuborish";
  ...
  switch (step) {
    ...
    case 'references':
      return Markup.inlineKeyboard([
        Markup.button.callback(SKIP_STEP_LABEL, 'skip:references'),
      ]);
    case 'resume':
      return Markup.inlineKeyboard([
        Markup.button.callback(SKIP_STEP_LABEL, 'skip:resume'),
      ]);
    default:
      return undefined;
  }
